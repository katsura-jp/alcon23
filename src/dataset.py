...
class AlconDataset2(Dataset):
    def __init__(self, df, augmentation, datadir, split=True, mode='train', onehot=True, margin_augmentation=False):
        super(AlconDataset2, self).__init__()
        self.df = df
        self.augmentation = augmentation
        self.datadir = datadir
        self.mode = mode
        ...
        self.num_classes = 48
        self.margin_augmentation = margin_augmentation
        self.validation_logits = torch.load("/mnt/hdd1/alcon2019/validation_logits.pth")

    def __getitem__(self, index):
        _index = self.df.index[index]
        image = np.array(Image.open(os.path.join(self.datadir, self.df.loc[_index, "file"])))
        if self.mode == 'train':
            logits = self.validation_logits[_index]
            targets = []
            for i in range(1, 4):
                label = int(self.df.loc[_index, f"target{i}"])
                if self.onehot:
            ...
            if self.split:
                ...
                for i, img in enumerate([img1, img2, img3]):
                    images.append(self.augmentation(image=img)['image'].numpy())
                images = torch.from_numpy(np.array(images))
                return images, targets, _index, logits
            else:
                image = self.augmentation(image=image)['image']
                return image, targets, _index, logits
