...
class FPInceptionV4GRU2(nn.Module):
    def __init__(self, num_classes, hidden_size=512, bidirectional=False, dropout=0.5, load_weight=None):
        super(FPInceptionV4GRU2, self).__init__()
        self.num_classes = num_classes
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional

        self.backbone = inceptionv4(num_classes=1000, pretrained='imagenet')
        self.features = self.backbone.features
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(p=0.5)

        if bidirectional:
            self.gru1 = nn.GRU(self.backbone.last_linear.in_features, hidden_size=hidden_size//2, batch_first=True,bidirectional=True)
