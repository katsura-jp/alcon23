...
def main():
    ...
    for fold in param['fold']:
        ...
        cycle_iter = 5
        # optim
        optimizer = torch.optim.SGD(model.parameters(), lr=param['lr'], momentum=0.9, weight_decay=1e-5, nesterov=False)

        # scheduler
        scheduler = CosineAnnealingWarmUpRestarts(optimizer, T_0=cycle_iter * len(train_dataloader), T_mult=1,
                                                  eta_max=0.1,T_up=500)
        ...
        for epoch in mb:
            ...
            for step, (inputs, targets, indices) in enumerate(progress_bar(train_dataloader, parent=mb)):
                ...
                if val_iter != -1 and step % val_iter == 0 and step != 0:
                    ...
                    logger.debug('============ epoch {} | iter {} | cycle {} =========='.format(epoch + 1, step+1, cycle))
                    logger.debug('lr              : {:.5f}'.format(scheduler.get_lr()[0]))
                    logger.debug('loss            : train={:.5f}  , test={:.5f}'.format(train_avg_loss, avg_valid_loss))
                    logger.debug(
                        'acc(per 1 char) : train={:.3%}  , test={:.3%}'.format(train_avg_accuracy, avg_valid_accuracy))
                    logger.debug('acc(per 3 char) : train={:.3%}  , test={:.3%}'.format(train_three_char_accuracy,
                                                                                        avg_three_valid_acc))
            ...
            logger.debug('============ epoch {} | iter {} | cycle {} =========='.format(epoch + 1, len(train_dataloader), cycle))
            logger.debug('lr              : {:.5f}'.format(scheduler.get_lr()[0]))
            logger.debug('loss            : train={:.5f}  , test={:.5f}'.format(train_avg_loss, avg_valid_loss))
            logger.debug(
                'acc(per 1 char) : train={:.3%}  , test={:.3%}'.format(train_avg_accuracy, avg_valid_accuracy))
